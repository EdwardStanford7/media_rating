        ...
        >
          ...
          {resetToken ?
            <form className={FORM_CLASS} onSubmit={handleResetPassword}>
              <PasswordField label="New password" name="newPassword" placeholder="New password" autoComplete="new-password" />
              <PasswordField
...
async function resetPasswordWithToken({ token, newPassword }: { token: string; newPassword: string }) {
...
function formatAuthError(error: unknown, mode: AuthMode, minPasswordLength: number) {
  ...
  if (
    ...
    (normalizedMessage.includes('character') || normalizedMessage.includes('length') || normalizedMessage.includes('short'))
  ) {
